...
\item 可加的逐点写入，支持精确的流式处理、层次化合并与分布式聚合，且无需重放分块内容，同时允许写入与读出完全并行；
...
PointNet 先做逐点变换再做最大池化~\cite{qi2017pointnet}，PointNet++ 在此基础上加入层次化度量邻域~\cite{qi2017pointnetplusplus}，Deep Sets 刻画了由求和构建的置换不变函数~\cite{zaheer2017deepsets}。三者都与点序无关，且 Deep Sets 使用的求和在式~\eqref{eq:add-cn} 的严格意义下是可加的。但不断增长的点流会撞上两处障碍。逐通道最大值丢弃了撤销它所需的信息，某个观测一旦被移除，已保存的摘要就无法再被修正。度量邻域是相对于当前持有的点定义的；插入或删除一个点，其他点所属的邻域也随之改变，分组只能重建。Deep Sets 绕开了这两处，代价是求和进一个单一的全局向量，它不携带空间地址，无法在某个被请求的坐标上求值。我们保留可加的求和，但把它铺展到 128 个带空间地址的行上，使坐标条件化的读取成为可能，同时不放弃可加性。

\subsection{基于学习中心的固定尺寸摘要}
VLAD 把局部描述子分配到学习到的中心并累加残差，从而压缩表示~\cite{jegou2010vlad}。NetVLAD 把该分配变为软分配且可微~\cite{arandjelovic2016netvlad}，PointNetVLAD 把这一构造用于点云的地点识别~\cite{uy2018pointnetvlad}。其累加器与我们的形状相同：一个由软分配到学习中心而构建的固定矩阵。差别在于累加什么，以及之后又对它做了什么。被累加的残差是一阶的，而式~\eqref{eq:value-cn} 保留了直到二阶的全部矩；分配是在特征空间中进行的，而我们的地址是坐标的归一化函数，对查询位置保持连续。决定性的差别在最后一步。NetVLAD 先对每个块做块内归一化，再对展平后的描述子做 $L_2$ 归一化，而这两步都不是可加的——把两个部分描述子各自归一化后再相加，并不能复现二者并集的描述子。原始累加器本身是可加的，但已发表的构造把这一性质归一化掉了，且从未使用它。我们让累加器保持未归一化，把归一化推迟到读出时刻，命题~\ref{prop:order-cn} 因此得以成立。
...
\subsection{本架构拒绝做的事}
上述每一族都为换取某项性质而交出了另一项：最大值交出可逆性，度量邻域交出编辑的局部性，归一化描述子交出可加性，softmax 编码器交出可分解性，序列化的循环交出顺序不变性。本文的设计源自写入时刻的一条拒绝：任何把某个点的写入与另一个点耦合起来的东西，都不得进入持久状态。这排除了在观测集合上的归一化、点与点之间的比较、选择与排序。聚合方法通常在吸收一个点时所做的运算——密度归一化、相对参考位置的中心化，以及非线性解释——在这里都被推迟到读出，从而不再触及所存矩阵的代数。表~\ref{tab:family-comparison-cn} 给出由此确定的位置。
...
Set Transformer，诱导点 & 是 & 否$^{b}$ & 是 & 否 \\
Perceiver IO，输出查询 & 是 & 否$^{b}$ & 是 & 是$^{c}$ \\
线性注意力、快速权重 & 是$^{d}$ & 是 & 是 & 否 \\
...
\end{tabular}
\\[3pt]
\parbox{\linewidth}{\footnotesize $^{a}$原始累加器是可加的，但块内归一化与 $L_2$ 归一化移除了该性质。$^{b}$在观测元素上的 softmax 把它们耦合在一起。$^{c}$Perceiver IO 从学习到的位置查询解码稠密输出，而非从输入空间中的坐标。$^{d}$外积更新本身与顺序无关，尽管这些模型被用于有序序列。}
\end{table}

这一拒绝带来了什么、又付出了什么，都是被测量出来的，而不是被断言的。第~\ref{sec:parallel-cn} 节隔离出并行状态结构的贡献，表~\ref{tab:segmentation_accuracy_cn} 与表~\ref{tab:classification_accuracy_cn} 则把由此得到的精度置于层次化基线之旁。
...
\subsection{并行求值}
\label{sec:parallel-eval-cn}
第 2.5 节的那条拒绝，同时也决定了计算可以如何铺开。在一个分块内部，某个点的写入不读取任何其他点，因此全部 $M$ 个点被同时求值，分块更新就是式~\eqref{eq:state-cn} 的一次矩阵乘积。在分块与设备之间，命题~\ref{prop:order-cn} 使合并成为求和，而求和满足结合律与交换律，因此部分状态可以并发产生并以任意次序归约，包括对 $D$ 个产生者做深度为 $\log D$ 的树形归约，其间无需同步、也无需固定调度。三个分支在写入过程中不共享任何项，它们的地址与值构成三个彼此独立的矩阵乘积，可以并发发起或批量处理。读出的情形相同：分割头对每个查询单独作用，从不在解码查询之间做池化，因此一组被请求的坐标同样是一次矩阵乘积加逐点网络，这些查询可以在设备之间划分而无需任何交换。

有一个细节使其中第一条成为可能。式~\eqref{eq:key-cn} 中的地址确实做了归一化，但归一化的是本分支自己的 128 个行，而不是观测集合，因此分母始终留在单个点自己的计算之内，从不变成跨点的归约。编码一个分块的关键路径于是不随其中点数增长，这也正是第~\ref{sec:additivity-cn} 节中实测的增量时延保持平坦、而重建时间随历史增长的原因。

有几种常见的构造不允许这种铺开方式。度量邻域需要在当前持有的点上做依赖数据的搜索，点集必须驻留，且访存是不规则的。对序列化后的点做循环，在序列维度上是顺序执行的，其结果依赖遍历方式，因此无法独立产生部分结果再合并。像 softmax 注意力那样在观测集合上取归一化分母，本身就是一次跨点归约，也就成了写入内部的一个同步点。
...
\end{tabular}
\end{table}
纯状态分类器达到 $86.59\%$ 的干净准确率，密度偏移下为 $82.47\%$。表~\ref{tab:classification_accuracy_cn} 把它与两个对照并列，并量化了“丢弃点表、只从可合并状态分类”所付出的代价。
...
在参数量不变的前提下，移除两个辅助状态使实例 mIoU 下降 $1.05$ 至 $1.20$ 个百分点。改为给某一个状态增加容量同样无济于事：在保留三个状态的前提下把主状态从 128 行加倍到 256 行，只达到 $74.79$，仍低于任何一个所有状态均为 128 行的配置。已发布模型使用三个状态、341,909 参数，在同一筛选协议下达到 $77.22$，比单状态对照高出 $2.54$ 个百分点，而参数量少 29.5\%。并行状态所贡献的可用结构，既不是更大的参数预算能提供的，也不是加宽主状态能提供的。这些是在数据子集上的短程筛选，作为受控比较报告，不作为最终精度。

\subsection{可加性与更新代价}
\label{sec:additivity-cn}
在 32 个物体上，由 2--32 个有序或打乱分块构建的状态与一次性构建的结果一致，相对误差低于 $5.3\times10^{-7}$。追加、合并、移除与衰减测试均低于 $9.8\times10^{-7}$。三个 float32 状态含 65,280 个标量（255 KiB），与点流长度无关。对短点云而言原始 XYZ 更小——实测的交叉点约为 21,800 个观测——其收益在于有界的保留内存，以及在长流或分布式场景中可直接进行的状态运算。
...
更确切地说，一个点一旦被写入，状态就是之后任何阶段唯一能看到的东西，因此没有任何解码器能够恢复写入时已经丢弃的区分。\textbf{状态而不是读出，才是该架构真正起约束作用的压缩瓶颈}。筛选实验直接支持这一点：改变分割解码器只使实例 mIoU 变化 $0.30\pm0.30$ 个百分点，改变其宽度或共享方式也只产生同量级的亚个百分点变化；而第~\ref{sec:parallel-cn} 节中改变状态配置则使其变化超过一个百分点。这正是本设计把结构预算花在“点如何被写入”——归一化地址、二阶值以及采用若干并行状态——而不是花在“状态如何被解码”上的原因，也解释了为什么加宽主状态无法替代并行结构。
...
\section{结论}
PointProgrammerNet 用固定的可加矩阵对点云作摘要。连续的键与二阶值支持无序写入、精确的分布式合并、移除、衰减，以及代价与历史长度无关的更新。若干并行状态所编码的可用结构多于同等参数预算下的单一状态，而真正起约束作用的压缩瓶颈是状态本身而非读出。坐标条件化的分割与纯状态的分类都在不对解码点做池化的情况下工作。该模型为“从有界流式状态出发的集中式或分布式点云推理”提供了一个紧凑且可检验的设计，其相对层次化基线的精度代价由实验予以量化。
