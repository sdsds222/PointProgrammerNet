...
% Converted from the Pattern Recognition Letters two-column version.
% Content is unchanged except for: (i) the supplementary material folded into
% the main text, (ii) a new Section 4.2 on parallel versus single states, and
% (iii) an explicit statement in Section 5.2 that the state, not the readout,
% is the binding compression bottleneck.
...
PointNet applies pointwise transforms followed by maximum pooling~\cite{qi2017pointnet}, PointNet++ adds hierarchical metric neighborhoods~\cite{qi2017pointnetplusplus}, and Deep Sets characterizes permutation-invariant functions built from sums~\cite{zaheer2017deepsets}. All three are invariant to point order, and the sum used by Deep Sets is additive in the strict sense of Equation~\eqref{eq:add-main}. Two properties still limit a growing stream. A channelwise maximum discards the information needed to undo it, so a stored summary cannot be corrected when an observation is later removed. A metric neighborhood is defined relative to the points currently held, so inserting or deleting one point changes which points belong to other neighborhoods and the grouping must be rebuilt. Deep Sets avoids both, but it sums into a single global vector that carries no spatial address and therefore cannot be evaluated at a requested coordinate. PointProgrammerNet keeps the additive sum and spreads it over 128 spatially addressed rows, which is what allows a coordinate-conditioned read without giving up additivity.

\subsection{Fixed-size summaries over learned centers}
VLAD compresses local descriptors by assigning them to learned centers and accumulating residuals~\cite{jegou2010vlad}. NetVLAD makes that assignment soft and differentiable~\cite{arandjelovic2016netvlad}, and PointNetVLAD applies the construction to point clouds for place recognition~\cite{uy2018pointnetvlad}. The accumulator has the same shape as ours: a fixed matrix built by soft assignment to learned centers. Three differences matter. First, the accumulated residuals are first order, whereas Equation~\eqref{eq:value-main} retains every moment up to second order. Second, the assignment is made in feature space, whereas our address is a normalized function of the coordinate and therefore remains continuous in a query position. Third, and most relevant here, NetVLAD intra-normalizes each block and then $L_2$-normalizes the flattened descriptor. Those steps are not additive: normalizing two partial descriptors and adding them does not reproduce the descriptor of their union. The raw accumulator is additive, but the published construction normalizes that property away and never uses it. We deliberately leave the accumulator unnormalized and move normalization to read time, which is exactly what preserves \Cref{prop:order}.
...
Set Transformer, Perceiver IO & yes & no$^{b}$ & yes & yes$^{c}$ \\
Linear attention, fast weights & sequence & yes & yes & no \\
...
\end{tabular}
\\[3pt]
\parbox{\linewidth}{\footnotesize $^{a}$The raw accumulator is additive, but intra-normalization and $L_2$ normalization remove the property. $^{b}$Softmax over the observed elements couples them. $^{c}$Perceiver IO decodes dense outputs from learned position queries rather than from a coordinate in the input space.}
\end{table}

The cost of that refusal is measured rather than asserted. \Cref{tab:segmentation_accuracy,tab:classification_accuracy} report the accuracy gap to hierarchical baselines, and \Cref{sec:parallel} shows that the parallel-state structure, and not the parameter budget, is what recovers part of it.
...
\end{tabular}
\end{table}
The state-only classifier reaches $86.59\%$ clean accuracy, 3.25 points below lightweight PointNet++ and 1.31 points below the parameter-matched PointNet. Its density-shift accuracy is $82.47\%$. This quantifies the accuracy cost of discarding the point list and classifying only from mergeable states.
...
\subsection{Where the representation is limited}
